...
DATASET_NAME_MAPPING = {
    ...
    "pickapic_v2": ("jpg_0", "jpg_1", "label_0", "caption"),
}
...
def parse_args():
    ...
    args.train_method = 'sft' if args.sft else 'ddiminpo'
    return args
...
def main():
    ...
    if args.train_method == 'ddiminpo': ref_unet.requires_grad_(False)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            ...
            if len(models) > 1:
                assert args.train_method == 'ddiminpo' # 2nd model is just ref_unet in DPO case
    ...
    if 'pickapic' in args.dataset_name or (args.train_method == 'ddiminpo'):
        pass
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.train_method == 'ddiminpo': # make timesteps and noise same for pairs in DPO
                ...
                with torch.no_grad():
                    ...
                    inversion_eta = 0.1 # adding random terms during the reverse process (optional)
                    for t in times:
                        ...
                        variance = noise_scheduler._get_variance(prev_t, t) ** (0.5)
                        noisy_latents += inversion_eta * torch.randn_like(noisy_latents,dtype=weight_dtype) * variance
                ...
                if args.train_method == 'sft': # SFT, F.mse_loss
                    loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
                elif args.train_method == 'ddiminpo':
